...
def test_export_writes_3dm(tmp_path):
    ...
    assert model.Strings["behop.provider"] == "BEhop"
    assert model.Strings["generated_by"] == "Generated by BEhop AI Tool"

    geometry = model.Objects[0].Geometry
    assert hasattr(geometry, "IsClosed")
    assert geometry.IsClosed is False
    assert model.Objects[0].Attributes.GetUserString("behop.service") == "RU-LineArt API"
    assert model.Objects[0].Attributes.GetUserString("geometry_type") == "nurbs"
...
def test_export_writes_multiple_curves(tmp_path):
    ...
    assert model.Objects[1].Attributes.GetUserString("behop.provider") == "BEhop"


def test_export_writes_polyline_curve(tmp_path):
    polyline = Polyline2D(points=[(0.0, 0.0), (30.0, 0.0), (30.0, 10.0)])

    out = tmp_path / "polyline.3dm"
    result = write_3dm(polyline, out, ExportConfig())

    assert result.output_path.exists()
    model = rhino3dm.File3dm.Read(str(out))
    assert model is not None
    assert len(model.Objects) == 1
    assert type(model.Objects[0].Geometry).__name__ in {"PolylineCurve", "LineCurve"}
    assert model.Objects[0].Attributes.GetUserString("geometry_type") == "polyline"


def test_export_writes_node_ids(tmp_path):
    polyline = Polyline2D(points=[(0.0, 0.0), (20.0, 0.0)])
    out = tmp_path / "polyline_nodes.3dm"

    _ = write_3dm(polyline, out, ExportConfig(), node_ids=(3, 9))
    model = rhino3dm.File3dm.Read(str(out))
    assert model is not None
    assert len(model.Objects) == 1
    attr = model.Objects[0].Attributes
    assert attr.GetUserString("start_node_id") == "3"
    assert attr.GetUserString("end_node_id") == "9"
